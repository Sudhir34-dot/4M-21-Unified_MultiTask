...
# 🔹 COCO Segmentation Dataset Class
class COCOSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for ann in annotations:
            bbox = ann["bbox"]
            if self.is_valid_bbox(bbox) and "segmentation" in ann:
        ...
        masks = torch.as_tensor(np.array(masks), dtype=torch.uint8)  # Convert masks to tensor
...
# 🔹 Load full dataset
full_dataset = COCOSegmentationDataset(image_folder, annotation_file, transform=transform)

# 🔹 Select 25,000 random images for training
random.seed(42)
selected_indices = random.sample(range(len(full_dataset)), 2500)
...
model_save_path = "C:/4m-21_image/models/mask_rcnn_coco10k.pth"
